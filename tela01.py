...
def apresentar():
    ...
    lista = view.apresentar_info(tabela)
    # Lista para cabeçalho
    tabela_head = ['ID', 'Nome', 'Data']
    
    # Criando a tabela como uma string formatada
    tabela_texto = f"{tabela_head[0]:<5} {tabela_head[1]:<20} {tabela_head[4]:<12} \n"
    
    for item in lista:
        tabela_texto += f"{tabela_head[0]:<5} {tabela_head[1]:<20} {tabela_head[4]:<12} \n"

    # Atualizando o conteúdo do rótulo
    labelgrade.config(text=tabela_texto)
